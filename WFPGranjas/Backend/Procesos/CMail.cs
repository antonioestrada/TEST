...
namespace WFPGranjas.Backend.Procesos
{
    class CMail
    {
        ...
        string DE = "antonio.estrada@dstx.mx"; //nuestro usuario de smtp
        string PASS = "PASS(jeaoeabz17)"; //nuestro password de smtp
        ...
        public bool enviaMail()
        {
            ...
            try
            {
                ...
                resultado = BeanCBeanParamMail.consultaParam("param_EMAIL1");
                //almacenamos datos en la variable global local
                //EMAIL1 es para SERVIDOR DE CORREO
                SmtpClient smtpMail = new SmtpClient(resultado.Pvalor);
                ...
                smtpMail.Host = resultado.Pvalor; //agregamos el servidor smtp
                resultado = BeanCBeanParamMail.consultaParam("param_EMAIL2");
                smtpMail.Port =int.Parse(resultado.Pvalor); //le asignamos el puerto, en este caso gmail utiliza el 465
                resultado = BeanCBeanParamMail.consultaParam("param_EMAIL3");
                DE = resultado.Pvalor;
                resultado = BeanCBeanParamMail.consultaParam("param_EMAIL4");
                PASS = resultado.Pvalor;

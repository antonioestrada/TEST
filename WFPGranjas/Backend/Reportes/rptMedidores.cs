...
namespace WFPGranjas.Backend.Reportes
{
    class rptMedidores
    {
        ...
        public void rptRegLecturas(CrystalReportViewer rptViewer, int op, string in_periodoAnt, string in_anioAnt)
        {
            ...
            while (reader.Read())
            {
                dsM.DTRegLecturas.AddDTRegLecturasRow(reader.GetValue(0).ToString(), reader.GetValue(1).ToString(), reader.GetValue(2).ToString(), reader.GetValue(3).ToString(), reader.GetValue(4).ToString(),Int32.Parse(reader.GetValue(5).ToString()), Int32.Parse(reader.GetValue(6).ToString()));
            }
            Conexion.FinalizarSesion();

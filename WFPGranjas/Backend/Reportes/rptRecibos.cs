...
namespace WFPGranjas.Backend.Reportes
{
    class rptRecibos
    {
        ...
        #region recibo de AGUA BONIFICACIONES
        public void rptReciboBonAgua(CrystalReportViewer rptViewer, int op, string inParam1, string inParam2, string inParam3)
        {
            Globales vGlobal = new Globales();
            ReportDocument cryRpt = new ReportDocument();
            ReportDocument crySubRpt = new ReportDocument();
            ...
            cryRpt = new ReportDocument();
            cryRpt.Load(@"" + vGlobal.pathReportes + "crReciboBonAg.rpt");
            crySubRpt.Load(@"" + vGlobal.pathReportes + "crReciboBonAgua.rpt");
            dsReciboMAnto dsR = new dsReciboMAnto();
            //***********ENCABEZADO DEL RECIBO*********************
            IDataReader reader = Conexion.GDatos.TraerDataReaderSql("CALL gestion_granjas.sp_report_ReciboAguaA(1,'" + inParam1 + "','" + inParam2 + "','" + inParam3 + "')");
            while (reader.Read())
            ...
            while (reader.Read())
            {
                dsR.DTRMPartidas.AddDTRMPartidasRow(Int32.Parse(reader.GetValue(0).ToString()), "BONIFICACION A CUOTAS DE AGUA", reader.GetValue(2).ToString(), reader.GetValue(3).ToString(), Decimal.Parse(reader.GetValue(4).ToString()));
            }
            Conexion.FinalizarSesion();
        ...
        #region GESTION de INGRESOS CASA CLUB
        public void rptGestionICC(CrystalReportViewer rptViewer, int op, string inParam1, string inParam2, string inParam3)
        {
            Globales vGlobal = new Globales();
            ReportDocument cryRpt = new ReportDocument();

            cryRpt = new ReportDocument();
            cryRpt.Load(@"" + vGlobal.pathReportes + "crNotificacionCuota.rpt");
            dsReciboMAnto dsR = new dsReciboMAnto();
            //***********ENCABEZADO DEL RECIBO*********************
            IDataReader reader = Conexion.GDatos.TraerDataReaderSql("CALL gestion_granjas.sp_report_ReciboMantoA(1,'" + inParam1 + "','" + inParam2 + "','" + inParam3 + "')");
            while (reader.Read())
            ...
            //***********DATOS DE PARTIDAS DE COBRO*********************
            reader = Conexion.GDatos.TraerDataReaderSql("CALL gestion_granjas.sp_report_ReciboMantoA(12,'" + inParam1 + "','" + inParam2 + "','0')");
            while (reader.Read())
            {
                dsR.DTRMPartidas.AddDTRMPartidasRow(Int32.Parse(reader.GetValue(0).ToString()), reader.GetValue(1).ToString(), reader.GetValue(2).ToString(), reader.GetValue(3).ToString(), Decimal.Parse(reader.GetValue(4).ToString()));
            }
            ...
            //***********DATOS FORMAS DE PAGO*********************
            reader = Conexion.GDatos.TraerDataReaderSql("CALL gestion_granjas.sp_report_ReciboMantoA(13,'" + inParam1 + "','" + inParam2 + "','" + inParam3 + "')");
            while (reader.Read())
            {
                dsR.DTRMPagos.AddDTRMPagosRow(Int32.Parse(reader.GetValue(0).ToString()), reader.GetValue(1).ToString(), Decimal.Parse(reader.GetValue(2).ToString()), reader.GetValue(3).ToString(), Decimal.Parse(reader.GetValue(4).ToString()), reader.GetValue(5).ToString(), Decimal.Parse(reader.GetValue(6).ToString()), reader.GetValue(7).ToString());
            }
           ...
           // string Folio = "";
            ...
            DiskFileDestinationOptions CrDiskFileDestinationOptions = new DiskFileDestinationOptions();
            PdfRtfWordFormatOptions CrFormatTypeOptions = new PdfRtfWordFormatOptions();
            CrDiskFileDestinationOptions.DiskFileName = @"" + vGlobal.pathReportesPDF + "notificaciones\\cuotas\\notificacionCuota.pdf";
            // CrDiskFileDestinationOptions.DiskFileName = "c:\\firebird\\reporteMail.pdf";
            CrExportOptions = cryRpt.ExportOptions;
            {
                CrExportOptions.ExportDestinationType = ExportDestinationType.DiskFile;
        ...
        #region recibo de INGRESOS CASA CLUB PUBLICO GRAL
        public void rptReciboICCGral(CrystalReportViewer rptViewer, int op, string inParam1, string inParam2, string inParam3)
        {
            Globales vGlobal = new Globales();
            ReportDocument cryRpt = new ReportDocument();
            ReportDocument crySubRpt = new ReportDocument();
            ...
            //***********DATOS DE PARTIDAS DE COBRO*********************
            reader = Conexion.GDatos.TraerDataReaderSql("CALL gestion_granjas.sp_report_ReciboMantoA(11,'" + inParam1 + "','" + inParam2 + "','0')");
            while (reader.Read())
            {
                dsR.DTRMPartidas.AddDTRMPartidasRow(Int32.Parse(reader.GetValue(0).ToString()), reader.GetValue(1).ToString(), reader.GetValue(2).ToString(), reader.GetValue(3).ToString(), Decimal.Parse(reader.GetValue(4).ToString()));
            }
        ...
        public void rptGestionCMedidor(CrystalReportViewer rptViewer, int op, string inParam1, string inParam2, string inParam3)
        {
            ...
            cryRpt = new ReportDocument();
            cryRpt.Load(@"" + vGlobal.pathReportes + "crNotificacionCambio.rpt");
            dsGestionesVarias dsR = new dsGestionesVarias();
            //***********ENCABEZADO DEL RECIBO*********************
            IDataReader reader = Conexion.GDatos.TraerDataReaderSql("CALL gestion_granjas.sp_report_GestionesVarias(1,'" + inParam1 + "','" + inParam2 + "','" + inParam3 + "')");
            while (reader.Read())
            {
                dsR.DTGVEncabezado.AddDTGVEncabezadoRow(Int32.Parse(reader.GetValue(0).ToString()), reader.GetValue(1).ToString(), reader.GetValue(2).ToString(), reader.GetValue(3).ToString(), reader.GetValue(4).ToString(), reader.GetValue(5).ToString(), reader.GetValue(6).ToString());
            }
            Conexion.FinalizarSesion();

            //***********DATOS DE COLONO*********************
            reader = Conexion.GDatos.TraerDataReaderSql("CALL gestion_granjas.sp_report_GestionesVarias(2,'" + inParam1 + "','" + inParam2 + "','" + inParam3 + "')");
            while (reader.Read())
            {
                dsR.DTGVColono.AddDTGVColonoRow(Int32.Parse(reader.GetValue(0).ToString()), reader.GetValue(1).ToString(), reader.GetValue(2).ToString(), reader.GetValue(3).ToString(), decimal.Parse(reader.GetValue(4).ToString()));
            }
            Conexion.FinalizarSesion();

            //***********DATOS DE PARTIDAS DE PARAM_N*********************
            reader = Conexion.GDatos.TraerDataReaderSql("CALL gestion_granjas.sp_report_GestionesVarias(3,'" + inParam1 + "','" + inParam2 + "','0')");
            while (reader.Read())
            {
                dsR.DTGVParam.AddDTGVParamRow(Int32.Parse(reader.GetValue(0).ToString()), reader.GetValue(1).ToString(), reader.GetValue(2).ToString(), reader.GetValue(3).ToString(), reader.GetValue(4).ToString(), reader.GetValue(5).ToString(), reader.GetValue(6).ToString(), reader.GetValue(7).ToString());
            }
            Conexion.FinalizarSesion();
